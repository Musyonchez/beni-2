...
- **NFR2: Security Requirements**
  All users must authenticate via Firebase Authentication before accessing any system function. Firestore security rules shall enforce role-based access: students may only read and write their own orders, pre-orders, and wallet records; staff may read and update all orders and top up wallets; no client may directly modify another user's wallet balance without a server-side Firestore transaction. Wallet deductions shall use Firestore runTransaction() to guarantee atomicity and prevent double-spending.
...
- **NFR6: Maintainability**
  All Firestore interactions shall be centralised in a FirestoreRepository class, so that query logic can be updated in one place without modifying Activity or Fragment code. Business logic for pre-order scheduling shall reside exclusively in Firebase Cloud Functions, making it independently deployable and testable without a device.
